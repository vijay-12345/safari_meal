...
class RestaurentController extends \App\Http\Controllers\Controller 
{
	...
	* restaurentList
	*/
	public function restaurentList()
	{
		...
		try{
			if(isset($inputs['long']) && isset($inputs['lat'])){
				...
				if(!empty($inputs['restaurantlimit'])){			
					$restaurantlimit=$inputs['restaurantlimit'];
				}else{
					$restaurantlimit=5;
				}		
				...
				$restaurant_query = Restaurent::select('timing.*','restaurant.*', \DB::raw('POW(69.1 * (latitude - '. $inputs['lat'] .'), 2) + POW(69.1 * ('. $inputs['long'] .' - longitude) * COS(latitude / 57.3), 2) AS distance'))
					->leftJoin('timing', 'restaurant.id', '=', 'timing.restaurant_id')
					->with('city', 'area', 'state', 'country', 'image', 'cuisines', 'reviewRating')
					->where('timing.weekday',$day_number_of_week)
					->whereIn('restaurant.id',$productResIds)->where('restaurant.status', 1)->whereraw('POW(69.1 * (latitude - '. $inputs['lat'] .'), 2) + POW(69.1 * ('. $inputs['long'] .' - longitude) * COS(latitude / 57.3), 2)<='.\Config::get('constants.search_m'));
				...
				$adminData = User::select('first_name','last_name','contact_number','email')->where(['role_id'=>Config::get('constants.user.super_admin')])->get();
	...
	*/
	...
	public function restaurantDetail()
	{
		...
		$id=	$inputs['id'];
		$restaurantDetails = [];
		$restaurantDetails = Restaurent::lang()->with('image')->with('area')->with('city')->with('state')->with('country')->with('openTime')->with(['review'=>function($query){
			$query->take(2);
		}])->with('reviewRating')->find($id);
