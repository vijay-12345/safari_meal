...
class HomeController extends Controller 
{
	...
	*/
	...
	{		
		$countries = Country::select('*')->where('country_id','=',101)->get();
	...
	public function tablebookingHistory()
	{
		if ( !Auth::check()){
			return redirect('/');		    
		}
		...
		$perPage = 20;
                $orders = DB::table('table_booking as o')
			->leftJoin('restaurant as r', 'o.restaurant_id', '=', 'r.id')
			->where('o.user_id', Auth::user()->id)
			->where('o.status', '!=', 0)
			->orderBy('o.date', 'desc')
		...
		return view('tablebookhistory')->with('orders', $orders);
	}
}
