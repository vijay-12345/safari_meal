	...
	*/
	...
	public function index(Request $request)
	{
		...
		*/
		...
		if($request->has('paginate_limit')) {
			...
			if($request->input('search'))
				...
				{
					$data = null;
					return view('admin.table.index',['data'=>$data,'filter'=>$filter]);
				}
		...
		*/
		$data = [];
		//end of chart
		$order_q = Table::where('table_booking.status','!=',\Config::get('constants.order.noconfirm'))
			->leftjoin('restaurant', 'table_booking.restaurant_id', '=', 'restaurant.id')
			->leftjoin('user', 'table_booking.user_id', '=', 'user.id')
			->select('table_booking.*','restaurant.name as restaurant_name','user.first_name as customer_first_name','user.last_name as customer_last_name')
		...
		if(Session::has('access.role') && Session::get('access.role') !='admin') {		
			$order_q->whereIn('table_booking.restaurant_id', Session::get('access.restaurant_ids'));
		}

		// if( Session::get('filter.status') == 'open' ) {
		// 	$order_q->where('table_booking.status', '!=', \Config::get('constants.order.delivered'));
		// }
		...
		if(Session::has('filter.search') && session('filter.search') !='') {
			$order_q->where('table_booking.order_number', 'like', '%'.session('filter.search'));
			session(['filter.custom_date' => '']);
			session(['filter.date' => 'all']);
		} else {
			if(Session::has('filter.date') && session('filter.date') !='all') {
				$order_q->whereDate('table_booking.date','=', session('filter.date'));
			} else if(Session::has('filter.custom_date') && session('filter.custom_date') !='') {
				$order_q->whereDate('table_booking.date','=', session('filter.custom_date'));
			}
		}
		
		$data['data'] = $order_q->whereNull('table_booking.deleted_at')->paginate(session('filter.paginate_limit'));
		//---------------------End of fetch data--------------------------
		...
		//dd($data['data']); 
        if($request->ajax()) {
            $viewHtml =  view('admin.table.ajaxtable',['data'=>$data,'filter'=>$filter]);
        	echo $viewHtml;die;
        } else {
        	return view('admin.table.index',['data'=>$data,'filter'=>$filter]);
        }	
	...
	public function add(Request $request)
	{
		...
		return view('admin.table.add',['data' => $data]);
	}
	...
	**/
	...
	public function edit($id) {
		...
		try {
			...
			$order_q = Table::leftjoin('restaurant', 'table_booking.restaurant_id', '=', 'restaurant.id')
			->select('table_booking.*','restaurant.name as restaurant_name')
			->where('table_booking.id',$id);
			...
			$orderdetails = (new TABLE())->getorderfulldetails($data['order']->id);
			...
			/*if($request['contactNumber']) {
	 		...
	 		return view('admin.table.edit',['data'=>$data,'type'=>'edit','customer'=>$customer, 'orderdetails'=>$orderdetails]);
	...
	public function update($id, Request $request){
	    $order = Table::findOrFail($id);
	    $input = $request->all();
    	
	    if (isset($input['radiog_lite'])) Table::setCustomerAdderss($input['radiog_lite'],$input['order_id']);
	...
	public function getOrdersCount()
	{
		...
		if (Auth::check() && Auth::user()->role_id == 6) {
			$ordersCount = Table::Where('status', 1)
		                    ->whereIn("restaurant_id", Session::get('access.restaurant_ids'))->whereNull('deleted_at')->count();
		} else {
			$ordersCount = Table::where('status', '1')->whereNull('deleted_at')->count();
		}
