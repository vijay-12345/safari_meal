...
class HomeCookedController extends \App\Http\Controllers\Controller 
{
	/*
	|--------------------------------------------------------------------------
	| Welcome Controller
	...
	*/
		...
		*/	
		...
		if($request->has('paginate_limit')) {
			...
			if($request->input('search'))
				...
				{
					$data = null;
					return view('admin.homecooked.index',['data'=>$data,'filter'=>$filter]);
				}
		...
		$q->where('restaurant.is_home_cooked','=', 1);				
	...
	public function add(Request $request) {
		...
		if($request->isMethod('post')) {
			...
			if(Session::get('access.role') == 'admin') {
		   	...
		   	$validator = Validator::make($request->all(), array_merge([
			        ...
			        'phone' => 'required|numeric|digits_between:9,10|unique:restaurant',
			        'cuisines'=>'required',
		...
		return view('admin.homecooked.add',['data'=>$data]);
	}
	...
	public function edit($id) {
		try {
	    	...
	    	return view('admin.homecooked.edit',['restaurant'=>$restaurant,'cuisineIdArr'=>$cuisineIdArr]);
	...
	public function delete($id, Request $request){
		...
		return redirect($request->segment(2).'/homecooked');
	}
	...
	{	
		try {
			...
			if(!empty($restaurantObj->owner_id)) {			
				return view('admin.homecooked.access',['restaurantObj'=>$restaurantObj,'user'=>$user]);
			} else {
				return view('admin.homecooked.access',['restaurantObj'=>$restaurantObj]);
			}
