...
class WelcomeController extends \App\Http\Controllers\Controller 
{
	...
	*/
	...
	public function updateOrderStatus($id, Request $request) {
		...
		// $this->notifyUser($order1['order_number'],$request->get('status'));
	...
	public function updateTableStatus($id, Request $request) {
		//echo $id;exit();
		Table::where('id', $id)->update($request->only(['status']));
	...
	public function update(Request $request)
	{
		...
		if($status1 == 2){
			if ($getOrder) $dataReturn = Order::notifyUserAboutOrderStatusOpenDriver($getOrder->toArray(), $getOrder->id);
		...
		if($status1 == 4)
		{
			...
			if ($getOrder) $dataReturn = Order::notifyUserAboutOrderStatusPickup($getOrder->toArray(), $getOrder->id);
	...
	//  public function notifyUser($orderNo,$status)
	// {
