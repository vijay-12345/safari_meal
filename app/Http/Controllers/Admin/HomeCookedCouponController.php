...
class HomeCookedCouponController extends \App\Http\Controllers\Controller 
{
	use PushNotify;
	...
	*/
		...
		*/
		...
		if($request->has('paginate_limit')) {
			...
			if($request->input('search'))
				...
				{
					$data = null;
					return view('admin.homecookedcoupon.index',['data'=>$data,'filter'=>$filter]);
				}
		...
		$q = Coupon::select('coupon.*','restaurant.name as restaurant_name','restaurant.is_home_cooked')
					...
					->where('restaurant.is_home_cooked','=','1')
	...
	public function add(Request $request)
	{
		...
		return view('admin.homecookedcoupon.add',['customers'=>$customers]);
	}
	...
	public function edit($id) {
		...
		try {
	    	...
	    	return view('admin.homecookedcoupon.edit',['customers'=>$customers,'coupon'=>$coupon]);
	...
	public function delete($id,Request $request) {
		...
		return redirect($request->segment(2).'/homecookedcoupon');
	}	
