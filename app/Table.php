...
class Table extends AppModel
{
	...
	public $fillable = ['id', 'date', 'time', 'user_id', 'amount', 'discount', 'commission', 'order_number', 'coupon_id', 'status', 'total_person', 'created_at', 'updated_at', 'restaurant_id', 'cgst', 'sgst', 'user_payment_method'];
	...
	{
		return $this->hasOne('App\Table','id','order_id');
	}
	...
	*/
	...
	public static function createOrder($requestArray, $cart)
	{
		...
		//prd($request);
		if (isset($requestArray['customer_id'])) {
			//prd($requestArray['customer_id']);	
			$customerContact = $requestArray['customer_id'];
			$customerData = User::select('user.id','user.first_name','user.last_name','user.contact_number')
					->Where('user.id', '=', $customerContact)->first();
		...
		if(count($cart->getData())==0) {
			...
			if(array_key_exists('search_customer', $request))	// backend request
			{
				...
				$order['total_person'] = trim($request['total_person']);
			...
			while(true) {
				...
				$orderNumber  = Table::where('order_number',$order_number)->first();
			...
			$orderid = Table::insertGetId($order);
	...
	public static function dateFormat(&$input,$format = 'Y-m-d H:i:s'){
 		if(isset($input['book_date'])){
			$date=date_create($input['book_date']);
			$input['book_date'] = date_format($date,$format);
 		} 	
    }
