...
$q = Restaurent::lang()->where('is_home_cooked',0)->where('status',1);
				...
				<div class="page-data-right">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => $prefix.'/review')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>
									...
									<td>
										<a href="{{url($prefix.'/review')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a>
									</td>
								</tr>
							</table>
							{!! Form::close() !!}
					    ...
					    <thead>
					        <tr>					           
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/review?sorting={{$filter['sort']}}&amp;field=restaurant.name">{{trans('admin.restaurant')}}</a></th>
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/review?sorting={{$filter['sort']}}&amp;field=user.first_name">{{trans('admin.customer')}}</a></th>
					             <th><a href="{{URL::to('/')}}/{{$prefix}}/review?sorting={{$filter['sort']}}&amp;field=review.review">{{trans('admin.review')}}</a></th>
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/review?sorting={{$filter['sort']}}&amp;field=review.rating">{{trans('admin.rating')}}</a></th>
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/review?sorting={{$filter['sort']}}&amp;field=review.date">{{trans('admin.date')}}</a></th>
					    ...
					    <tbody>
					    	...
					    	@if($data != null && count($data['data']) > 0)
						        ...
						        <tr id="list_data{{$v->id}}" @if($i%2==0) style="background-color: #f9f9f9;" @else style="background-color: #FFFFFF;" @endif>					        	
						            ...
						            <td><a href="{{url($prefix.'/review').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="review" style="cursor:pointer;"></span></td>
						        </tr>       
