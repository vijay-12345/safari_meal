				...
				<div class="page-data-right">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<!-- vijayanand -->
					        @if($roleid == 6)
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => 'restaurant/menu')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>
									<td>
										<select name="paginate_limit" class="paginate_limit">							
									...
									<td>
										<a href="{{url('restaurant/menu')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a>
									</td>
								</tr>
							</table>
							{!! Form::close() !!}
					    ...
					    <thead>
					        <tr>					           
					            ...
					            <!-- vijayanand -->   
					            @if($roleid == 1)
					            <th><a href="{{URL::to('/')}}/admin/menu?sorting={{$filter['sort']}}&amp;field=restaurant_id">{{trans('admin.restaurant_name')}}</a></th>
					            @endif
						        ...
						        <tr id="list_data{{$v->id}}" @if($i%2==0) style="background-color: #f9f9f9;" @else style="background-color: #FFFFFF;" @endif>					        	
						            ...
						            <td>@if($v->status == 1)
						            	...
						            	@else
						            		{{trans('admin.inactive')}}
						            	@endif</td>								
						            <td><span class="glyphicon glyphicon-plus-sign view-action" data-id="{{$v->id}}" style="cursor:pointer;"></span>&nbsp;&nbsp;<a href="{{url('restaurant/menu').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="menu" style="cursor:pointer;"></span></td>
						        </tr>						        
						        <tr style="display:none;" id="view_data{{$v->id}}">
						        	<td colspan="8">
						        		<table class="table-responsive" cellspacing="0" width="100%">
