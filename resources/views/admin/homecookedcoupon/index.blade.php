				...
				<div class="page-data-right">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => $prefix.'/homecookedcoupon')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>									
									...
									<td>
										<a href="{{url($prefix.'/homecookedcoupon')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a>
									</td>
								</tr>
							</table>
							{!! Form::close() !!}
					    	...
					    	<thead>
					        <tr>					           
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=restaurant.name">{{trans('admin.restaurant.name')}}</a></th>
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=coupon_code">{{trans('admin.coupon.code')}}</a></th>
					             <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=coupon_value">{{trans('admin.coupon.value')}}</a></th>
					           
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=start_date">{{trans('admin.start.date')}}</a></th>
					            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=end_date">{{trans('admin.end.date')}}</a></th>
					             <th><a href="{{URL::to('/')}}/{{$prefix}}/homecookedcoupon?sorting={{$filter['sort']}}&amp;field=status">{{trans('admin.status')}}</a></th>
							        ...
							        <tr id="list_data{{$v->id}}" @if($i%2==0) style="background-color: #f9f9f9;" @else style="background-color: #FFFFFF;" @endif>					        	
							            ...
							            <td>{{ $endTime }}</td>				            
						             	<td>
							            	...
							            	@endif</td>
							            <td><a href="{{url($prefix.'/homecookedcoupon').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="homecookedcoupon" style="cursor:pointer;"></span></td>
							        </tr>       
						        @endforeach
					      	@else
