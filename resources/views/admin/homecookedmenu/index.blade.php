				...
				<div class="page-data-right">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => 'admin/homecookedmenu')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>
									...
									<td>
										<a href="{{url('admin/homecookedmenu')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a>
									</td>
								</tr>
							</table>
							{!! Form::close() !!}
					    ...
					    <thead>
					        <tr>					           
					            ...
					            <th><a href="{{URL::to('/')}}/admin/homecookedmenu?sorting={{$filter['sort']}}&amp;field=restaurant_id">{{trans('admin.restaurant_name')}}</a></th>
					            <th><a href="{{URL::to('/')}}/admin/homecookedmenu?sorting={{$filter['sort']}}&amp;field=name">{{trans('admin.name')}}</a></th>
					            <th><a href="{{URL::to('/')}}/admin/homecookedmenu?sorting={{$filter['sort']}}&amp;field=created_at">{{trans('admin.created')}}</a></th>
					            <th><a href="{{URL::to('/')}}/admin/homecookedmenu?sorting={{$filter['sort']}}&amp;field=status">{{trans('admin.status')}}</a></th>
					    	...
					    	@if($data != null && count($data['data']) > 0)
						        ...
						        <tr id="list_data{{$v->id}}" @if($i%2==0) style="background-color: #f9f9f9;" @else style="background-color: #FFFFFF;" @endif>					        	
						            ...
						            <td>@if($v->status == 1)
						            	...
						            	@endif</td>								
						            <td><span class="glyphicon glyphicon-plus-sign view-action" data-id="{{$v->id}}" style="cursor:pointer;"></span>&nbsp;&nbsp;<a href="{{url('admin/homecookedmenu').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="homecookedmenu" style="cursor:pointer;"></span></td>
						        </tr>						        
