...
@section('title', trans('admin.cuisine.list'))
@section('content')
			...
			<div class="col-md-9">
				<div class="page-data-right">
					<div class="panel panel-default">
						<div class="panel-heading">{{trans('admin.cuisine.manager')}}</div>
						<div class="panel-body">
							...
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => 'admin/cuisine')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>
									...
									<td>
										<a href="{{url('admin/cuisine')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a>
									</td>
								</tr>
							</table>
							{!! Form::close() !!}
					    ...
					    <thead>
					        <tr>					           
					            ...
					            <th><a href="{{URL::to('/')}}/admin/cuisine?sorting={{$filter['sort']}}&amp;field=name">{{trans('admin.name')}}</a></th>
					            <th><a href="{{URL::to('/')}}/admin/cuisine?sorting={{$filter['sort']}}&amp;field=created_at">{{trans('admin.created')}}</a></th>
					            <th><a href="{{URL::to('/')}}/admin/cuisine?sorting={{$filter['sort']}}&amp;field=status">{{trans('admin.status')}}</a></th>
					    ...
					    <tbody>
					    	...
					    	@if($data != null && count($data['data']) > 0)
						        ...
						        <tr id="list_data{{$v->id}}" @if($i%2==0) style="background-color: #f9f9f9;" @else style="background-color: #FFFFFF;" @endif>					        	
						            ...
						            <td><span class="glyphicon glyphicon-plus-sign view-action" data-id="{{$v->id}}" style="cursor:pointer;"></span>&nbsp;&nbsp;<a href="{{url('admin/cuisine').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="cuisine" style="cursor:pointer;"></span></td>
						        </tr>						        
						        <tr style="display:none;" id="view_data{{$v->id}}">
						        	<td colspan="8">
						        		<table class="table-responsive" cellspacing="0" width="100%">
						        			<tr>
						        				<td><strong>{{trans('admin.cuisine.id')}} :</strong> {{ $v->id }}</td>						        				
						        			...
						        			<tr>						
						        				<td><strong>{{trans('admin.created')}} :</strong> {{ ucfirst($v->created_at) }}</td>
						        				<td><strong>{{trans('admin.updated')}} :</strong> {{ ucfirst($v->updated_at) }}</td>
						        				<td>&nbsp;</td>
						         			</tr>	          			       			
						        		</table>
						        	</td>	
