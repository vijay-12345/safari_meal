				...
				<div class="page-data-right">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{!! Form::open(array('role' => 'form','class'=>'filter-form','url' => $prefix.'/homecooked')) !!}
							<table class="table table-responsive filter-inner-form" cellspacing="0" width="100%">
								<tr>
									...
									@if(Session::get('access.role') == 'admin')
										<td><a href="{{url($prefix.'/homecooked')}}/add" class="btn btn-primary btn-md">+ {{trans('admin.add')}}</a></td>
									@endif
								</tr>
							</table>
							{!! Form::close() !!}
						    ...
						    <thead>
						        <tr>
						            ...
						            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecooked?sorting={{$filter['sort']}}&amp;field=name">{{trans('admin.name')}}</a></th>
						            <th>{{trans('admin.address')}}</th>					 
						            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecooked?sorting={{$filter['sort']}}&amp;field=restaurant.rating">{{trans('admin.rating')}}</a></th>
						            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecooked?sorting={{$filter['sort']}}&amp;field=restaurant.featured">{{trans('admin.featured')}}</a></th>					           
						            <th><a href="{{URL::to('/')}}/{{$prefix}}/homecooked?sorting={{$filter['sort']}}&amp;field=restaurant.status">{{trans('admin.status')}}</a></th>					           
					    	...
					    	@if($data != null && count($data['data']) > 0)					    		
						        ...
						        <tr>					        	
						            ...
						            <td>
						            	...
						            	<a href="{{url($prefix.'/homecooked').'/access/'.$v->id}}" class="glyphicon glyphicon-user" <?php if($v->owner_id >0) {?> style="color:#006400;" <?php } ?> alt="{{trans('admin.restaurant.credential')}}"></a>&nbsp;&nbsp;
						            	@endif
						            	<a href="{{url($prefix.'/homecooked').'/edit/'.$v->id}}"><span class="glyphicon glyphicon-pencil" style="cursor:pointer;" data-id="{{$v->id}}"></span></a>&nbsp;&nbsp;
						            	<a href="{{url($prefix.'/timing').'/'.$v->id}}" class="glyphicon glyphicon-time" alt="Open Timing" title="Open Timing"></a>&nbsp;&nbsp;
						            	@if(Session::get('access.role') == 'admin')
						            		<span class="glyphicon glyphicon-remove delete-action" data-id="{{$v->id}}" data-controller="homecooked" style="cursor:pointer;"></span>
						            	@endif
						            </td>
						        </tr>
						        @endforeach
